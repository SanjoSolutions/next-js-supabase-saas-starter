...
describe("NotificationCenter", () => {
  ...
  it("shows 9+ when unread count exceeds 9", async () => {
    const manyUnread = Array.from({ length: 12 }, (_, index) => ({
      id: `notif-${index}`,
  ...
  it("does not show badge when all notifications are read", async () => {
    const allRead = mockNotifications.map((notification) => ({
      ...notification,
      is_read: true,
    }))
  ...
  it("highlights unread notifications with different background", async () => {
    ...
    await waitFor(() => {
      const unreadNotification = screen
        .getByText("New member joined")
        .closest("[role='menuitem']")
      expect(unreadNotification).toHaveClass("bg-accent/50")

      const readNotification = screen
        .getByText("Subscription updated")
        .closest("[role='menuitem']")
      expect(readNotification).not.toHaveClass("bg-accent/50")
    })
  })
